...
public class ValidationNestedErrorTest {
    ...
    public void testHashCode() {
        ...
        final String FIELD = "field";
        final NestedError nestedError = new ValidationNestedError(FIELD, MESSAGE);
        final NestedError nestedErrorNull = new ValidationNestedError();

        int hashExpected = MESSAGE.hashCode();
        hashExpected = 31 * hashExpected + (FIELD.hashCode());

        final int hashResult = nestedError.hashCode();

        assertEquals(hashExpected, hashResult);
        assertEquals(0, nestedErrorNull.hashCode());
    }
}